...
feature --Handlers
	...
	-- Remove a project
		local
			l_user_email, l_project_name: STRING
			l_result_payload: JSON_OBJECT
		do
	...
	-- Rename a project
		...
		do
			...
			if attached {JSON_OBJECT} parser.parse as j_object and parser.is_parsed then
				...
				-- we have to convert the json string into an eiffel string
				--if attached {JSON_STRING} j_object.item ("old_project_name") as s then
									--l_old_project_name := s.unescaped_string_8
				--end
				if attached {JSON_STRING} j_object.item ("new_project_name") as s then
	...
	-- Get all the members and owners of a project
		...
		do
			...
			elseif my_db.check_project_name (l_project_name) = false then
				...
				-- Get from the database all the projects which the user takes part in: db.get_all_user_project(a_user_email)
				l_result_payload := (my_db.get_all_project_members (l_project_name))
				-- Message tutto bene
				set_json_header_ok (res, l_result_payload.representation.count)
			end
	...
	-- Add a member or an owner to a project
		...
		do
			...
			elseif my_db.is_member (l_user_email, l_project_name) = false then
				...
				-- Message tutto bene
				l_result_payload.put (create {JSON_STRING}.make_json ("New member '" + l_new_member + "' added successfully to '" + l_project_name + "'."), create {JSON_STRING}.make_json ("Success"))
				set_json_header_ok (res, l_result_payload.representation.count)
			end
