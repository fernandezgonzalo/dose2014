...
feature --Handlers
	...
	-- Rename a project
		...
		do
			...
			elseif l_new_project_name.count > 40 then
				...
				-- Message tutto bene
				l_result_payload.put (create {JSON_STRING}.make_json ("New project '" + l_new_project_name + "' added successfully."), create {JSON_STRING}.make_json ("Success"))
				set_json_header_ok (res, l_result_payload.representation.count)
			end
	...
	-- Get all the members and owners of a project
		local
			l_payload, l_project_name: STRING
			j_obj: JSON_OBJECT
			l_result_payload: JSON_ARRAY
			parser: JSON_PARSER
		do
